...
This topic is explained in further detail in the Security page, but we'll do a brief recap here.
...
the LiveComponent vulnerable to someone who is savvy enough to execute the AJAX requests. So we'll use some `IsGranted` attributes on the live actions.

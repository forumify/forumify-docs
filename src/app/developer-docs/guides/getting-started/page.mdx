...
Let’s start this up so we can create a database for forumify, and a user with restrictive permissions that the platform can use. Run the following command:
...
<Alert severity="info">
    If you want to use a GUI client from your computer to connect to your server, you should connect through an SSH tunnel to your sever, most clients will have this option, and then connect to localhost:3306.
</Alert>
...
Let’s create the database named `forumify`, and a user named `forumify` that can access it, take note of the user’s password because you will need it soon.

```docker
$ mysql -uroot -proot -h127.0.0.1

mysql> CREATE DATABASE forumify;
Query OK, 1 row affected

mysql> CREATE USER 'forumify'@'localhost' IDENTIFIED BY '<new user password>';
Query OK, 0 rows affected

mysql> GRANT ALL PRIVILEGES ON forumify.* TO 'forumify'@'localhost';
Query OK, 0 rows affected
...
Finally, you should change the root password to something else as well. Make sure you take note of it somewhere in case you need it in the future.
